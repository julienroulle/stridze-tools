...
speed = st.number_input(
    f"VMA ({dist_unit}/h)", min_value=0.0, max_value=30.0, value=19.4
)

pace_speed_tab, short_distance_tab, long_distance_tab, race_time_tab = st.tabs(
    ["Pace and Speed", "Short Distance", "Long Distance", "Race Times"]
)
...
with pace_speed_tab:
    ...
    speeds = speed * vma_percentages / 100.0
    ...
    df = pd.DataFrame(
        {
            ...
            f"Paces (min/{dist_unit})": [
                f"{pace // 60:.0f}:{pace % 60:02.0f}" for pace in paces
            ],
        }
    )
    df = df.set_index("VMA")
...
with short_distance_tab:
    ...
    times = {
        f"{distance}m": [
            f"{pace * distance / 1000 // 60:.0f}min{pace * distance / 1000 % 60:02.1f}s"
            for pace in paces
        ]
...
with long_distance_tab:
    ...
    times = {
        f"{distance}m": [
            f"{pace * distance / 1000 // 60:.0f}min{pace * distance / 1000 % 60:02.0f}s"
            for pace in paces
        ]
        for distance in distances
    }
...
with race_time_tab:
    ...
    distances = [5.0, 10.0, 21.097, 42.195]
    paces = pace / (vma_percentages / 100)
    times = {
        f"{distance}km": [
            f"{pace * distance // 3600:.0f}h{pace * distance % 3600 // 60:.0f}min{pace * distance % 60:02.0f}s"
            for pace in paces
        ]
        for distance in distances
    }
