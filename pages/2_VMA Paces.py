...
speed = st.number_input(f"VMA ({dist_unit}/h)", min_value=0.0, max_value=30.0, value=15.0)
...
with pace_speed_tab:
    ...
    vma_percentages = np.arange(60, 120, 5)
...
with race_time_tab:
    ...
    vma_percentages = np.arange(65, 100, 5)
    distances = [5.000, 10.000, 21.097, 42.195]
    paces = pace / (vma_percentages / 100)
    times = {
        f"{distance}km": [f"{pace * distance // 60:.0f}min{pace * distance % 60:02.0f}s" for pace in paces] for distance in distances
    }
    df = pd.DataFrame(times, index=[str(elt) + ' %' for elt in vma_percentages])
