...
df['I Km'] = df.apply(lambda row: row['I Km'] if row['I Km'] != -1.0 else row['I 400m'] * 2.5, axis=1)
df['R Km'] = df.apply(lambda row: row['R 800m'] * 1.25 if row['R 800m'] != -1.0 else row['R 600m'] * 5 / 3 if row['R 600m'] != -1.0 else row['R 400m'] * 2.5 if row['R 400m'] != -1.0 else row['R 200m'] * 5, axis=1)
df = df.drop(columns=['T 400m', 'I 400m', 'I 1200m', 'I Mile', 'R 200m', 'R 300m', 'R 400m', 'R 600m', 'R 800m'])
